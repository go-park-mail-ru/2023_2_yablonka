	...
	return &LocalUserStorage{
		userData: map[string]*entities.User{
			...
			"newchallenger@email.com": {
				...
				Name:         "Major",
				Surname:      "Guile",
				AvatarURL:    "https://sun1-56.userapi.com/s/v1/ig2/tZBD5-zfhGO0BMktMHmj82YYqRowSPJbj7ZZE2lQ33DO1WzXB7z3fISjgAWaccX0marlKBf6tV_x0ScnO7CdM_ay.jpg?size=50x0&quality=96&crop=192,40,539,539&ava=1",
			},
			"ghostinthem@chi.ne": {
				ID:           4,
				...
				Name:         "Lain",
				Surname:      "Iwakura",
				AvatarURL:    "https://sun1-57.userapi.com/s/v1/ig2/YWU6FC5ElvSSShdkrSEqWC6InGJfDQv2yuWQREEFBpHt9Nsvs_o9qc3rR2yAyVdTGy7MLMaaGbOj1DhY33JWS-b7.jpg?size=50x50&quality=95&crop=439,244,605,605&ava=1",
			},
		},
		mu: &sync.RWMutex{},
	}
...
// или возвращает ошибку apperrors.ErrUserNotFound (401)
func (s *LocalUserStorage) GetUserByID(ctx context.Context, id uint64) (*entities.User, error) {
	defer s.mu.RUnlock()
...
func (s *LocalUserStorage) Create(ctx context.Context, info dto.SignupInfo) (*entities.User, error) {
	s.mu.RLock()
	_, ok := s.userData[info.Email]
	s.mu.RUnlock()
...
func (s *LocalUserStorage) Update(ctx context.Context, updatedUser entities.User) (*entities.User, error) {
	s.mu.RLock()
	oldUser, ok := s.userData[updatedUser.Email]
	s.mu.RUnlock()
	...
	s.mu.Lock()
	s.userData[updatedUser.Email] = &updatedUser
	s.mu.Unlock()
...
// или возвращает ошибку apperrors.ErrUserNotFound (409)
func (s *LocalUserStorage) Delete(ctx context.Context, id uint64) error {
	var (
		userEmail string
		ok        bool = false
	)
